...
export const selectTaskEpic: RootEpic = ( 
    ...
    return action$.pipe(
        ...
        roleMap(store$,
            (data: any) => task.selectTask4Api(data.payload, store$.value),
            (data: any) => task.selectTask4Local(data.payload, store$.value)
        ),
...
export const autoSaveTaskEpic: RootEpic = (
    ...
    return action$.pipe(
        ...
        map((value) => {
            console.log(value);
            console.log(store$);
            return actions.autoSaveDisableAction()
        })
    );
  };

// 开启自动保存
export const enableAutoSave: RootEpic = ( 
    action$, 
) => {
    return action$.pipe(
        filter(isActionOf(actions.selectTaskAutoSave)),
        map((action) => actions.autoSaveEnableAction())
    )
};
// 关闭自动保存
export const disableAutoSave: RootEpic = ( 
    action$, 
) => {
    return action$.pipe(
        filter(isActionOf(actions.selectTaskAutoSave)),
        map((action) => actions.autoSaveDisableAction())
    )
};